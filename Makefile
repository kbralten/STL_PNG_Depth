...
test-fast:
	...
	@if [ -f "pb/bin/activate" ]; then \
		. pb/bin/activate && cd test && python test_parallel.py --fast; \
	else \
		cd test && python test_parallel.py --fast; \
	fi
...
test-parallel:
	...
	@if [ -f "pb/bin/activate" ]; then \
		. pb/bin/activate && cd test && python test_parallel.py; \
	else \
		cd test && python test_parallel.py; \
	fi
...
test-serial:
	...
	@if [ -f "pb/bin/activate" ]; then \
		. pb/bin/activate && cd test && python test_simple.py; \
	else \
		cd test && python test_simple.py; \
	fi
...
benchmark:
	...
	@if [ -f "pb/bin/activate" ]; then \
		. pb/bin/activate && cd test && python benchmark.py; \
	else \
